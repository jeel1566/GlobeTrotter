...
const BUZZ_FILTERS = [
  { id: 'all', label: 'All' },
  { id: 'instagram', label: '🔥 Instagram', icon: Flame },
  ...
  return (
    <AppShell>
      <div className="flex flex-col w-full min-h-screen bg-[#FDFBF7]/60 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100">
        ...
        <section className="sticky top-0 z-30 w-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-b border-slate-200/80 dark:border-slate-800 shadow-xs">
          <div className="max-w-[1720px] mx-auto px-4 md:px-6 py-2.5 flex flex-col gap-2">

            {/* Area Search Bar */}
            <form onSubmit={handleSearchSubmit} className="flex items-center gap-2">
              <div className="flex-1 flex items-center bg-slate-100 dark:bg-slate-800/80 rounded-xl px-3.5 py-2 border border-slate-200/80 dark:border-slate-700/80 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500 transition-all">
                <MapPin className="w-4 h-4 text-blue-600 mr-2.5 shrink-0" strokeWidth={1.75} />
                <input
                  type="text"
                  value={searchInput}
                  onChange={(e) => setSearchInput(e.target.value)}
                  placeholder="Search city or area (e.g. Ahmedabad, Shibuya, Paris)..."
                  className="w-full bg-transparent text-sm font-semibold text-slate-900 dark:text-white focus:outline-none truncate placeholder:text-slate-400 placeholder:font-normal"
                />
                {isLoading ? (
              ...
              <Button
                type="submit"
                disabled={isLoading}
                className="rounded-xl px-4 h-10 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs gap-1.5 shadow-xs shrink-0"
              >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 overflow-x-auto pb-0.5 scrollbar-none">
              {/* Preset buttons */}
        ...
        <AnimatePresence>
          {addedToast && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="fixed top-20 right-6 z-50 p-3.5 rounded-xl bg-emerald-600 text-white text-xs font-semibold flex items-center gap-2.5 shadow-lg border border-emerald-500"
            >
        ...
        <div className="flex flex-col lg:flex-row w-full max-w-[1720px] mx-auto flex-1 p-4 md:p-6 gap-6">
          ...
          <div className="w-full lg:w-[48%] flex flex-col gap-3.5 lg:h-[calc(100vh-140px)] lg:overflow-y-auto pr-1">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Trending spots aggregated from Instagram & Reddit
                </p>
              </div>
